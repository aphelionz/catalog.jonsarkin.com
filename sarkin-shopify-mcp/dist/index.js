...
createPage.initialize(shopifyClient);
// Set up MCP server
...
server.tool("update-page", {
    id: z.string().min(1).describe("Shopify page GID, e.g. gid://shopify/Page/123"),
    title: z.string().optional(),
...
server.tool("create-page", {
    ...
    const result = await createPage.execute(args);
    return {
        content: [{ type: "text", text: JSON.stringify(result) }],
    };
});
