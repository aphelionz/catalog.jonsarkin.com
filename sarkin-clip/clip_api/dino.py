...
def embed_segment(image: "Image.Image", mask: np.ndarray, bbox: tuple) -> List[float]:
    """Embed a single segment using DINOv2 CLS token.

    Applies the mask (gray fill outside), crops to bbox, and returns
    the L2-normalized CLS token (768-dim).

    Args:
        image: PIL Image (RGB)
        mask: boolean ndarray (H, W) — True inside segment
    ...
    # Apply mask: gray (128) background outside segment
    ...
    device, model, transform = _get_dino()
    tensor = transform(masked_image).unsqueeze(0).to(device)

    with _dino_encode_lock, torch.no_grad():
        features = model.forward_features(tensor)
        cls_token = features["x_norm_clstoken"]  # [1, 768]
        cls_token = F.normalize(cls_token, dim=-1)
        result = cls_token[0].cpu().tolist()
    # Explicitly free large tensors to prevent OOM in batch runs
    del features, cls_token, tensor, masked, crop_array, gray_bg

    return result


def embed_segment_query(image_bytes: bytes) -> List[float]:
    """Embed a query crop using DINOv2 CLS token (no masking).

    For segment-level search: the query is a crop that already contains
    the motif of interest. Returns 768-dim L2-normalized CLS token.
    """
    from PIL import Image
    import torch
    import torch.nn.functional as F
